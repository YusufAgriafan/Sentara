...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Tambah Cerita Baru</h1>
                <p class="text-gray-600 mt-1">Buat cerita menarik untuk tempat wisata sejarah</p>
            </div>
    ...
    <!-- Form -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form action="{{ route('educator.stories.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <select id="historical_id" 
                    ...
                    @foreach($places as $place)
                        <option value="{{ $place->id }}" 
                                ...
                                {{ old('historical_id') == $place->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea id="content" 
                          ...
                          placeholder="Tulis cerita yang menarik dan edukatif. Ceritakan sejarah, legenda, atau fakta menarik tentang tempat ini...">{{ old('content') }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
