...
@section('page-title', 'Edit Data Sejarah')

@section('content')
<!-- Head                    <select name="subcategory" id="subcategory" 
...
<div class="bg-white rounded-xl shadow-lg border border-gray-200 mb-8">
    <div class="px-4 sm:px-6 lg:px-8 py-4 sm:py-6 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div class="min-w-0">
                <h1 class="text-2xl sm:text-3xl font-bold text-primary mb-2">Edit Data Sejarah</h1>
                <p class="text-sm sm:text-base text-gray-600 truncate">Perbarui konten: {{ $history->title }}</p>
            </div>
...
<div class="bg-white rounded-xl shadow-lg border border-gray-200">
    <form action="{{ route('admin.history.update', $history) }}" method="POST" class="p-4 sm:p-6 lg:p-8">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                <div>
                    ...
                    <input type="text" name="title" id="title" 
                           ...
                           value="{{ old('title', $history->title) }}" placeholder="Masukkan judul sejarah" required>
                    @error('title')
                ...
                <div>
                    ...
                    <input type="number" name="order_index" id="order_index" min="0"
                           ...
                           value="{{ old('order_index', $history->order_index) }}" required>
                    @error('order_index')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                            <i class="fas fa-exclamation-circle mr-1"></i>
                            {{ $message }}
                ...
                <div>
                    ...
                    <select name="category" id="category" 
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category }}" {{ old('category', $history->category) == $category ? 'selected' : '' }}>
                                {{ $category }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="mb-8">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10"
                          ...
                          placeholder="Masukkan konten sejarah dengan detail yang lengkap...">{{ old('content', $history->content) }}</textarea>
                @error('content')
                    <p class="mt-2 text-sm text-red-600 flex items-center">
                        <i class="fas fa-exclamation-circle mr-1"></i>
                        {{ $message }}
...
<script>
    ...
    const existingSubcategory = '{{ old('subcategory', $history->subcategory) }}';
    const existingSubSubcategory = '{{ old('sub_subcategory', $history->sub_subcategory) }}';
    ...
    function updateSubcategories() {
        ...
        if (category && subcategories[category]) {
            subcategories[category].forEach(function(subcategory) {
                ...
                if (existingSubcategory === subcategory) {
    ...
    function updateSubSubcategories() {
        ...
        if (subcategory && subSubcategories[subcategory]) {
            subSubcategories[subcategory].forEach(function(subSubcategory) {
                ...
                if (existingSubSubcategory === subSubcategory) {
                    option.selected = true;
                }
...
$(document).ready(function() {
    ...
    console.log('Summernote initialized for edit page');
});
